...
import pygame
from classes.ammo.bullet import LightBullet
...
class Shotgun:
    ...
    def __init__(self, game, pos):
        ...
        self.original_image = pygame.image.load(os.path.join("imgs", "shotgun.png"))
...
class ShotgunItem(Shotgun):
    ...
    def attack_event(self, game, pos):
        if self.ammo["current"] > 0:
            game.projectiles.append(LightBullet(game, pos, game.mouse_angle, "player"))
            game.projectiles.append(LightBullet(game, pos, game.mouse_angle - 0.12, "player"))
            game.projectiles.append(LightBullet(game, pos, game.mouse_angle - 0.24, "player"))
            game.projectiles.append(LightBullet(game, pos, game.mouse_angle + 0.12, "player"))
            game.projectiles.append(LightBullet(game, pos, game.mouse_angle + 0.24, "player"))
            self.ammo["current"] -= 1
